...
export default function RecordModal({ record, customPositions, onSave, onClose, onAddCustomPosition }) {
  const initForm = () => {
    ...
    return {
      ...
      intern_type: record.intern_type || INTERNSHIP_TYPES[0],
      position_type: record.position_type || DEFAULT_POSITIONS[0],
      custom_position: record.custom_position || '',
      submit_date: record.submit_date || null,
      has_exam: record.has_exam || '无',
      exam_date: record.exam_date || null,
      exam_status: record.exam_status || '待定',
      interview1_date: record.interview1_date || null,
      interview1_status: record.interview1_status || '待定',
      interview2_date: record.interview2_date || null,
      interview2_status: record.interview2_status || '待定',
      interview3_date: record.interview3_date || null,
      interview3_status: record.interview3_status || '待定',
    }
  }
      ...
      <div style={s.modal} onClick={e => e.stopPropagation()}>
        ...
        <div style={s.grid2}>
          ...
          <div style={{ ...s.fg, gridColumn:'1/-1' }}>
            <label style={s.label}>岗位链接</label>
            <input style={s.input} value={form.job_url} onChange={e => set('job_url', e.target.value)} placeholder="选填，粘贴招聘链接，点击可直接跳转"/>
          </div>
        </div>
